...
class IdToken implements IdTokenInterface
{
     ...
     */
    public function __construct(UserClaimsInterface $userClaimsStorage, PublicKeyInterface $publicKeyStorage, $at_string, array $config = array(), EncryptionInterface $encryptionUtil = null)
    {
        $this->userClaimsStorage = $userClaimsStorage;
        $this->publicKeyStorage = $publicKeyStorage;
        $this->at_string = $at_string;
    ...
    public function createIdToken($client_id, $userInfo, $nonce = null, $userClaims = null, $access_token = null)
    {
        ...
        list($user_id, $auth_time) = $this->getUserIdAndAuthTime($userInfo);
        $access_token = $this->at_string;
        
        //JoseD
        $Claims = $this->userClaimsStorage->getUserClaims($user_id, str_replace(' openid','',$_SESSION['scope']));
        $count = 0;
        foreach ($Claims as $isKey => $isValue){
            if ($count<5 and $count>=0){ //How many claims to set
                $userClaims[$isKey] = $isValue;
            }
            $count += 1;
        }
