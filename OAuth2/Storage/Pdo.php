    ...
    OpenIDAuthorizationCodeInterface
{
    ...
    public function getBolsa($bolsaid)
    {
        $stmt = $this->db->prepare($sql = sprintf('SELECT * from %s where bolsaid=:bolsaid', $this->config['bolsas_table']));
        $stmt->execute(array('bolsaid' => $bolsaid));

        if (!$bolsaInfo = $stmt->fetch(\PDO::FETCH_ASSOC)) {
            return false;
        }
    ...
    public function setBolsa($bolsaid, $enroll_verify, $valueSaldo, $valueField, $dateTime)
    {
        if ($enroll_verify == '1'){
            $stmt = $this->db->prepare($sql = sprintf('UPDATE %s SET saldo=:valueSaldo, EnrollCount=:valueField, ActualizacionFecha=:dateTime where bolsaid=:bolsaid', $this->config['bolsas_table']));
        }
        if ($enroll_verify == '2'){
            $stmt = $this->db->prepare($sql = sprintf('UPDATE %s SET saldo=:valueSaldo, VerifyCount=:valueField, ActualizacionFecha=:dateTime where bolsaid=:bolsaid', $this->config['bolsas_table']));
        }
        return $stmt->execute(compact('bolsaid', 'valueSaldo', 'valueField', 'dateTime'));
    }
     ...
     * @return bool
     */
    public function setRecord($userV2)
    {
        $JsonR = null;
        $pila = array();
        $string1 = "INSERT INTO ado_records (";
        foreach($userV2 as $sbCampo=>$sbValor){
            ${$sbCampo} = $sbValor;
            $pila[] = $sbCampo;
            $string1 .= $sbCampo.", ";
        }
        $pila[] = 'JsonR';
        $string1 .= "JSON_Response) VALUES (";
        foreach($userV2 as $sbCampo=>$sbValor){
            $string1 .= ":".$sbCampo.", ";
        }
        $string1 .= ":JsonR)";
        ...
        $string2 = "";
        foreach($Extras as $sbCampo=>$sbValor){
            $string2 .= $sbCampo.":".$sbValor.", ";
        }
        $Extras = substr($string2, 0, -2);
    
    	$string3 = "";
        foreach($Response_ANI as $sbCampo=>$sbValor){
            $string3 .= $sbCampo.":".$sbValor.", ";
        }
        $Response_ANI = substr($string3, 0, -2);
    
        $stmt = $this->db->prepare($sql = $string1);

        return $stmt->execute(compact($pila)); 
    }
    
    public function getRecordUpdate($TID){
        $stmt = $this->db->prepare('SELECT newParameters from actualizaciones where TransactionId=:TID GROUP BY actualizaId HAVING MAX(actualizaId)');
        $stmt->execute(array('TID' => $TID));

        if (!$updateInfo = $stmt->fetch(\PDO::FETCH_ASSOC)) {
            return false;
        }
    ...
    public function getRecord($TID){
        if ($TID){
            $stmt = $this->db->prepare('SELECT * from ado_records where TransactionId=:TID');
            $stmt->execute(array('TID' => $TID));
        	$recordInfo = $stmt->fetch(\PDO::FETCH_ASSOC);
        }
        else {
            $Extras = 'IdState:1, StateName:Pendiente';
            $stmt = $this->db->prepare('SELECT TransactionId from ado_records where Extras=:Extras');
            $stmt->execute(array('Extras' => $Extras));
            $recordInfo = $stmt->fetchAll(\PDO::FETCH_ASSOC);
        }
    ...
    private function setActualizacion($TID, $oldParameters, $newParameters, $observacion = null)
    {
        $update = $this->getRecordUpdate($TID);
        if ($update){
            $oldParameters = $update['newParameters'];
        }
        
        $stmt = $this->db->prepare('INSERT INTO actualizaciones (oldParameters, newParameters, TransactionId, observacion) VALUES (:oldParameters, :newParameters, :TID, :observacion)');
        return $stmt->execute(compact('oldParameters','newParameters','TID','observacion'));
    }

    public function updateRecord($data, $fields)
    {
        $TransactionId = $data['TransactionId'];
        $pila = array('TransactionId');
        
        $record = $this->getRecord($TransactionId);
        $VerifyUpdate = intval($record['VerifyUpdate'])+1;

        $string1 = "UPDATE ado_records SET ";
    	$stringUpdate = '';
        foreach($data as $sbCampo=>$sbValor){
            if (in_array($sbCampo, $fields)){
                ${$sbCampo} = $sbValor;
                $pila[] = $sbCampo;
                $string1 .= $sbCampo."=:".$sbCampo.', ';
            	$stringUpdate .= $sbCampo.":".$sbValor.', ';
            }
        }
    	$stringUpdate = substr($stringUpdate, 0, -2);

        if (in_array('JSON_Response', $fields)){
            $JsonR = json_encode($data);
            $pila[] = 'JsonR';
            $string1 .= "JSON_Response=:JsonR, VerifyUpdate=:VerifyUpdate where TransactionId=:TransactionId";
        }
        else {
            $string1 .= "VerifyUpdate=:VerifyUpdate where TransactionId=:TransactionId";
        }
        $pila[] = 'VerifyUpdate';
        ...
        $string4 = "";
        foreach($Images as $sbCampo=>$sbValor){
            $string4 .= $sbCampo.":".$sbValor.", ";
        }
        $Images = substr($string4, 0, -2);

        $string5 = "";
        foreach($SignedDocuments as $sbCampo=>$sbValor){
            $string5 .= $sbCampo.":".$sbValor.", ";
        }
        $SignedDocuments = substr($string5, 0, -2);

        $string6 = "";
        foreach($Scores as $sbCampo=>$sbValor){
            $string6 .= $sbCampo.":".$sbValor.", ";
        }
        $Scores = substr($string5, 0, -2);
        
        $stmt = $this->db->prepare($sql = $string1);

    	$oldString = json_encode($this->getRecord($TransactionId));
        $this->setActualizacion($TransactionId, $oldString, $stringUpdate);
     ...
     * @return bool
     */
    public function setUserV3($userV3)
    {
        $IdNumber = $userV3['IdNumber'];
        $givenName = $userV3['FirstName']." ".$userV3['SecondName'];
        $familyName = $userV3['FirstSurname']." ".$userV3['SecondSurname'];
        $name = $givenName." ".$familyName;
        $UID = $userV3['Uid'];
        $TransactionId = $userV3['TransactionId'];
        $picUrl = 'http://18.207.243.253/bridge_destinatario/bodega/records/'.$IdNumber.'/'.$TransactionId.'/clientFace.png';
    	$email = $userV3['email'];
        
        // if it exists, update it.
        if ($this->getUser($IdNumber)) {
            $string = "UPDATE oauth_users SET name=:name, given_name=:givenName, family_name=:familyName, picture=:picUrl, Uid=:UID, TransactionId=:TransactionId, email=:email where username=:IdNumber";
        } 
        else {
            $string = "INSERT INTO oauth_users (username, name, given_name, family_name, picture, Uid, TransactionId, email) VALUES (:IdNumber, :name, :givenName, :familyName, :picUrl, :UID, :TransactionId, :email)";
        }
        $stmt = $this->db->prepare($sql = $string);
        
        return $stmt->execute(compact('IdNumber','name','givenName','familyName','picUrl','UID','TransactionId','email'));
    }
    
    public function getEmail($document)
    {
    	$stmt = $this->db->prepare($sql = 'SELECT usu_email from usuario where usu_docume=:document');
        $stmt->execute(array('document' => $document));
