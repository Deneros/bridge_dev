    ...
    UserInfoControllerInterface
{
    ...
    protected function createDefaultIdTokenResponseType()
    {
        ...
        //Generar token y almacenarlo en JSON.
        $UIC = $this->storages['user_credentials'];
        $usern = array_keys($UIC->userCredentials)[0];
        $at = $this->getAccessToken_v2();
        $fp = fopen("ATJ/".strval($usern).".json", 'w');
        fwrite($fp, json_encode(array('AT'=>$at)));
        fclose($fp);
        
        return new IdToken($this->storages['user_claims'], $this->storages['public_key'], $at, $config);
    }
    ...
    protected function getAccessToken_v2()
    {
